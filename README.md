...
### AoA 推定: サブキャリアスムージング MUSIC

2×2 MIMO (RX アンテナ 2 本) では物理的なアンテナ数が不足し、通常の MUSIC アルゴリズムでは角度分解能が著しく低い。本システムでは **サブキャリアスムージング** 技術により仮想的にアンテナ数を拡大し、この制約を克服する。

#### 原理

OFDM の隣接サブキャリア間の位相勾配は、物理アンテナ間の位相差と数学的に等価な構造を持つ。この性質を利用し、サブキャリア軸方向にスライディングウィンドウ（サイズ $L$）を適用すると、各ウィンドウ位置で $(n_{rx} \times L)$ 次元のスナップショットベクトルが得られる。これにより **仮想アンテナ数** を $n_{rx} \times L$ に拡大できる。

物理配列: [RX0] [RX1] → 2素子 (分解能: 低)

サブキャリアスムージング後: Window 1: [RX0_sc1 .. RX0_sc20] [RX1_sc1 .. RX1_sc20] Window 2: [RX0_sc2 .. RX0_sc21] [RX1_sc2 .. RX1_sc21] ... Window K: [RX0_scK .. RX0_sc(K+19)] [RX1_scK .. RX1_sc(K+19)] → 仮想 40 素子 (160MHz, L=20 の場合)


#### Forward-Backward Averaging

コヒーレント信号（壁反射のような相関の強いマルチパス）のデコヒーレンスのため、各スナップショットに対して反転共役ベクトルも共分散行列に加算する:


$$R_{FB} = \frac{1}{2K} \sum_{k=1}^{K} \left( \mathbf{s}_k \mathbf{s}_k^H + J \mathbf{s}_k^* \mathbf{s}_k^T J \right)$$

ここで $J$ は逆順単位行列（exchange matrix）。

#### 帯域幅と仮想アンテナ数の対応

| 帯域幅 | サブキャリア数 | ウィンドウ $L$ | 仮想アンテナ数 | 角度分解能 |
|--------|--------------|---------------|--------------|-----------|
| 160 MHz | 468 | 20 | **40** | ≈ 2.5° |
| 80 MHz | 234 | 15 | **30** | ≈ 3.3° |
| 40 MHz | 114 | 10 | **20** | ≈ 5.0° |

160 MHz モードでは実効的に 40 素子アレイと同等の角度分解能が得られ、複数の反射パスを方位角レベルで分離可能。

#### 2D MUSIC (方位角 + 仰角)

方位角のみの 1D 推定に加え、方位角 $\theta \in [-90°, +90°]$ × 仰角 $\phi \in [-45°, +45°]$ の 2D グリッドサーチにも対応。天井・床方向の反射パスを含めた全方位の到来方向を推定できる。

#### マルチバンド AoA 融合

3 バンド (2.4 GHz / 5 GHz 80 MHz / 5 GHz 160 MHz) の AoA 推定結果を帯域幅に比例した重みで統合する。160 MHz の結果が最大の重みを持つ。方位角の差が ≈ 14° 以内のピークは同一パスとみなしてマッチングし、加重平均で融合する。複数バンドで一致したパスは信頼度がブーストされる。

#### AoA × ToF → 壁面位置特定

AoA（到来方向）と ToF（距離）を組み合わせることで、反射源の 3D 位置を特定し、最寄りの壁面に投影する。この情報は `reflection_map.py` の反射強度マップにガウスカーネルとして追加され、ToF のみの場合より高精度な壁内構造の位置特定が可能になる。

            AoA (方位角θ, 仰角φ)
                 ↗
計測点 ──────────→ 反射源 (3D位置) ↘ ToF (距離 d) ↓ 最寄り壁面に投影 → face, u座標, v座標 → 反射マップに重み加算
...
### Phase F-1 (完了)
- サブキャリアスムージング MUSIC による仮想アンテナ拡大 (2×2 MIMO → 仮想最大40素子)
- 2D MUSIC (方位角 + 仰角) による全方位到来方向推定
- 3バンド (2.4/5GHz_80/5GHz_160) AoA 融合 (帯域幅加重平均)
- AoA × ToF → 壁面位置マッピング (ガウスカーネルで反射マップに統合)
- 多指標信頼度評価 (SNR/ピーク鋭さ/アンテナファクター/パワーの4指標加重平均)
- FeitCSI データ形式の AoA 推定対応 (CSIFrame metadata 拡張)
- SimulatedAdapter に物理ベースアンテナ間位相差を実装
- AoA 統合テストスイート追加 (tests/test_aoa.py)
...
| **F-1** | AoA統合, サブキャリアスムージングMUSIC, マルチバンド融合 | ✅ 完了 |
